      ...
      var linkingFunction = function linkingFunction($scope, element, attr) {
        ...
        var selector = attr.selector
          ...
          , resetToMinDate = function resetToMinDate() {
            ...
            if($scope.dateTitles && $scope.dateTitles.month){
              $scope.month=$scope.dateTitles.month[$scope.month];
            }
           ...
           // console.log(JSON.parse($scope.datetitlesCn),$scope.dateTitles.month);
            ...
            if($scope.dateTitles ){
              ...
              if($scope.dateTitles.week){
                ...
                //  var wk= $filter('date')(new Date(new Date('06/08/2014').valueOf() + A_DAY_IN_MILLISECONDS * el), 'EEE');
                   // wk=$scope.dateTitles.week[wk];
                  return el; 
                });
              }
            }
          ...
          unregisterdatetitlesCNWatcher=$scope.$watch('datetitlesCn',function datetitlesCNWatcher(newValue){
           if(newValue){
            setLanguage();
           }
          })
        ...
        $scope.daysInString = $scope.daysInString.map(function mappingFunc(el) {
          ...
          var wk= $filter('date')(new Date(new Date('06/08/2014').valueOf() + A_DAY_IN_MILLISECONDS * el), 'EEE');
          if($scope.dateTitles && $scope.dateTitles.week){
            wk=$scope.dateTitles.week[wk];
          }
          return wk;
        });
